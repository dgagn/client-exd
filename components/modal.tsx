...
export default function Modal({ open, toggleOpen }: ModalProps) {
    ...
    const confirmation = useMemo(() => {
        return <>Êtes-vous sure de vouloir créer la liste {(<span className='color-info'>{trim(nom)}</span>)} ?</>
    }, [nom])
    ...
    return open ? (
            <div className='modal'>
                <div className="content rounded relative">
                    ...
                    <h3 className='mt-sm'><span role='img' aria-label='emoji pour un étoile'>⭐</span> Création d&apos;une liste personnelle</h3>
                    <p className='mt-sm'>Les listes sont utilisées pour classer vos recherches</p>
                    <span className='block mt-lg'>
                        <label htmlFor="nom" className='form__label'>Nom de la liste</label>
                        <input type="text" id='nom' className='form__control' onChange={handleChange} value={nom} />
                        <p className='text-sm text-contrast-500 mt-xs'>Un maximum de 30 caractères</p>
                    ...
                    <span className='block mt-sm'>
                        ...
                        <textarea onChange={handleNoteChange} value={note} id='note' className='form__control' />
                    </span>
                    <p className='mt-sm'>{confirmation}</p>
                    <div className="flex mt-md mb-sm align-center justify-center">
                        <button className='button-reset text-bg-fx text-bg-fx--scale-y mr-lg' onClick={newToggle}>Annuler</button>
                        {nomError === '' && <button className='button-reset text-bg-fx text-bg-fx--scale-y button-bg px-md py-sm rounded h-50' onClick={createList}>Créer la liste</button>}
                    </div>
                </div>
            </div>
    ) : null;
